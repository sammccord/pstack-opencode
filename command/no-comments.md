...
Load the `no-comments` skill with `skill({ name: "no-comments" })` and execute it.
...
Follow the `no-comments` skill exactly as written. Do not improvise around its steps or
...
- `/review` for a full diff review
- `/unslop` for prose surfaces
