...
argument-hint: "<behavior to pin with tests>"
---

# TDD

Load the `tdd` skill with `skill({ name: "tdd" })` and execute it.
...
Follow the `tdd` skill exactly as written. Do not improvise around its steps or
...
- `/blast-radius` to find what else the change touches
