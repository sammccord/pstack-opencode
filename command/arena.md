...
argument-hint: "<design or implementation to run a bakeoff on>"
---

# Arena

Load the `arena` skill with `skill({ name: "arena" })` and execute it.
...
Follow the `arena` skill exactly as written. Do not improvise around its steps or
...
- `/interrogate` to adversarially review the winner
- `/architect` if no entrant is good enough
