...
Load the `typescript-best-practices` skill with `skill({ name: "typescript-best-practices" })` and execute it.
...
Follow the `typescript-best-practices` skill exactly as written. Do not improvise around its steps or
