...
This skill orchestrates three others: an inline mining pass (see step 1), opencode's built-in the **authoring-a-skill** playbook (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Capture this session's transcript with
...
A `-mode` skill is subjective output. A the **authoring-a-skill** playbook-style test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.
