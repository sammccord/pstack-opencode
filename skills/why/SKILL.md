...
Before spawning investigators, list the available MCPs from the opencode environment. Use the available-tools map when present. Otherwise inspect the `mcps/` directory opencode exposes for enabled MCP servers.
...
Spawn each investigator with `subagent_type: pstack-fast-read`.
...
Spawn one synthesizer subagent with `subagent_type: pstack-judgment-read`.
