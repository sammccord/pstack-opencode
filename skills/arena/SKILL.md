...
3. Pick the runners. Default to one each on `pstack-judgment`, `pstack-precise`,
...
Spawn all N subagents in one message, each with the task, the path to the shared
...
After all Phase B candidates complete, pick one judge from `pstack-judgment-read`,
