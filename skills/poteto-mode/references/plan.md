...
State your read of scope and constraints in one paragraph. Use `question` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Prefer `subagent_type: "poteto-agent"`. `pstack-fast-read` is the fallback. Never use the built-in `plan` subagent_type; it ignores this skill.
- Pass `model:` explicitly per the configured roles (defaults `pstack-fast` for code, `pstack-judgment` for judgment).
...
If a phase creates or edits a skill, the phase instructs the implementer to use the **authoring-a-skill** playbook (`playbooks/authoring-a-skill.md`).
...
**Runtime.** Exercise the feature on the matching surface via the project's verification skill:

- Browser / Electron / Web UIs: the project's UI verification skill.
- CLIs and TUIs: the project's CLI verification skill.
- Native mobile: whatever simulator-driving skill your team has.
- No verification skill for the touched surface: flag it in the plan.
...
- `/deslopify` over each diff before commit. the **unslop** skill over any prose surface.
- the **show-me-your-work** skill to keep a decision trail when the plan is large enough to need an auditable record.
- opencode's built-in **babysit** skill after opening the PR.
