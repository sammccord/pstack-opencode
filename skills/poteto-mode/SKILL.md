...
- Before commit → the **deslopify** skill (`/deslopify`).
...
**In opencode, model choice is which subagent you pick.** `task` accepts
...
- **Autonomous run.** A long task to drive to completion without stopping ("run until done", "run until done"). `playbooks/autonomous-run.md`.
...
- **Pause safely.** Suspending in-flight work cleanly so it can be resumed, on an explicit pause, going offline, a opencode restart, or imminent context compaction. The complement to Session pickup. Full steps: `playbooks/pause-safely.md`.
