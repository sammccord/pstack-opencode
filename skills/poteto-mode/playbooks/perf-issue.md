...
1. Capture a baseline trace via the project's verification skill for that surface.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (default `pstack-precise`); review the diff. Capture a post-fix trace.
