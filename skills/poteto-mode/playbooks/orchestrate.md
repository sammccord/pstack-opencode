...
Create the store at `.opencode/orchestrate/<project-slug>/` inside the project, so it travels
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run in a dedicated worktree so they do not block the coordinator; at this scale an in-place restack takes the laptop down.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the subagent's returned report. Transcript mtime is not liveness.
...
- After a opencode restart: in-flight subagents are gone, but pushed branches and open PRs are not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach work by PR and branch rather than session id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
