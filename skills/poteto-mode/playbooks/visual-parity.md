...
4. Verify each component against its baseline via image diff on the matching surface via the project's verification skill. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. iterate per component until the diff is zero.
