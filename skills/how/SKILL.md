...
Spawn all explorers in a single message with `subagent_type: pstack-fast-read`.
...
Spawn a single subagent that explores and explains in one pass, with
...
Once all explorers return, spawn a single subagent to synthesize their findings into one
...
After the explanation is complete, spawn one architectural critic per panel agent, all in
a single message:
